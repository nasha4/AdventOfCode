...
public partial class Day05(bool isPart1) : IAdventPuzzle
{
    ...
    {
        var grid = new Grid.Helper(inputHelper.EachLineInSection(line => line));
        var stacks = Enumerable.Range(0, (grid.Max[1] + 2) / 4)
            .Select(x => grid.Ranges[0].Reverse().Select(y => grid[[y, x * 4 + 1]]).TakeWhile(c => c != ' ').ToList())
            .ToArray();
