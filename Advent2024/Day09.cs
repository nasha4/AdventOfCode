...
public class Day09(InputHelper inputHelper) : IAdventPuzzle
{
    public long Solve(bool isPart1)
    {
        ...
        if (isPart1)
        {
            ...
            while (node != tail)
            {
                ...
                tail.ValueRef.used -= 1;
                node.ValueRef.free -= 1;
                node.ValueRef.used += 1;
        ...
        return disk.Aggregate((sum: 0L, block: 0),
            (acc, term) => (acc.sum + (2L * acc.block + term.used - 1L) * term.used / 2L * term.fileId, acc.block + term.used + term.free)).sum;
    }
